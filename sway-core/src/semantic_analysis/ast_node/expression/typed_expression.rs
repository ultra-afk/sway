...
impl ty::TyExpression {
        ...
        let (typed_block, block_return_type) =
            ty::TyCodeBlock::type_check(handler, ctx.by_ref(), &contents).unwrap_or_else(|_| {
                (
                    ...
                    type_engine.insert(engines, TypeInfo::Tuple(Vec::new()), Some(&SourceId::primitive())),
                )
            });
    ...
    fn type_check_while_loop(
        ...
        let unit_ty = type_engine.insert(engines, TypeInfo::Tuple(Vec::new()), Some(&SourceId::primitive()));
